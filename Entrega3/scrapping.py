            ...
            if 'alerts' in data:
                for alerta in data['alerts']:
                    ...
                    eventos_locales.append({
                        ...
                        "x": x,
                        "y": y,
                        "timestamp": datetime.now(ZoneInfo("America/Santiago")).strftime("%Y-%m-%d %H:%M")
                    })
...
def guardar_eventos(eventos, base='tiempo'):
    # Buscar archivos existentes con el patrón "tiempoN.csv"
    existentes = [f for f in os.listdir('.') if re.match(rf'{base}\d+\.csv', f)]
    
    if existentes:
        numeros = [int(re.findall(r'\d+', f)[0]) for f in existentes]
        siguiente = max(numeros) + 1
    else:
        siguiente = 1

    nombre_archivo = f"{base}{siguiente}.csv"
